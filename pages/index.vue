...
const onClickLogin = async () => {
  ...
  const { data: resLogin, pending: pendingLogin, error } = await useFetch<Login>(`${baseURL.value}/auth/login`,
    {
      method: "POST",
      body: {
        email: email.value,
...
const decodeTokenBtn = async () => {
  const { data: tokenDec } = await useFetch<Login>(`${baseURL.value}/auth/refresh-token`,
    {
      method: "POST",
      body: {
